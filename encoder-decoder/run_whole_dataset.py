...
def read_nii(path):
    ...
    df = pd.read_csv("/anvme/workspace/b180dc42-ct-rate/data_volumes/valid_metadata.csv")
...
def main():
    ...
    if args.modal == "nii":
        ...
        for nii_file in tqdm.tqdm(nii_files[:200]):
            ...
            with torch.no_grad():
                ...
                decoded_patch = decoded_output_patch.detach().float().cpu().numpy()
                #outs.append(decoded_patch)

            decoded_patch = decoded_patch[0,0]
            print(decoded_patch.shape)
            affine = np.eye(4)
            img = nib.Nifti1Image(decoded_patch, affine)

            # Save as .nii.gz
            nib.save(img, os.path.join(output_dir,f"{nii_file.stem}.gz"))
