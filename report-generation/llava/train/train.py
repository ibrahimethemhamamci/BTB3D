    ...
    for source in sources:
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence['value']:
                #sentence["value"] = sentence["value"].replace("<report_generation>", "")
...
class LazySupervisedDataset(Dataset):
    ...
    def nii_img_to_tensor(self, path):
        ...
        """
        embedding = np.load(path)["arr"]
        embedding = embedding.transpose(0,2,3,4,1)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            image_file = image_file.replace(".nii.gz", ".npz").replace(".npz",".nii_embedded.npz" )
...
def train(attn_implementation=None):
    ...
    trainer = LLaVATrainer(model=model.cuda(),
                           tokenizer=tokenizer,
                           args=training_args,
